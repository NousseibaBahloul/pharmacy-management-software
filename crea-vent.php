...
if ($créer) {	
	if ($nuve=="" or $copr=="" or $quantité=="") {
    	echo '<script>alert("Entrer toutes les données");</script>';
    }else{
    	$query=mysql_query("SELECT * FROM `vente` WHERE `n_vente`='$nuve' AND `date_vente` =CURRENT_DATE() ");
    	$rows = mysql_num_rows($query);
    	if ($rows==1) {
    		echo '<script>alert("Cet n° de vente est existe déjà en cet jour,saisir autre numero");</script>';
    	}else{
	 	  	$query1=mysql_query("select * from produit where codeproduit = '$copr' ");
	    	$rows1 = mysql_num_rows($query1);
	    	if ($rows1==0) {
	    		echo '<script>alert("Ce produit n est existe pas déjà");</script>';
	    	}else{ 
		    	$query2=mysql_query("SELECT `quantité` FROM `produit` WHERE `codeproduit`='$copr'");
		    	$rows2 = mysql_fetch_object($query2);
		    	$qut=$rows2->quantité; 
	    		if ($qut<$quantité) {
	    			echo '<script>alert("Le quantité saisie est indisponible");</script>';
	    		}else{
	    			$query3=mysql_query("INSERT INTO `vente`(`n_vente`, `date_vente`, `total_ppa`) VALUES ('$nuve',CURRENT_DATE(),'')");
	    	    	if ($query3) {
				    	$query4=mysql_query("INSERT INTO `vente_produit` VALUES ('$nuve','$copr','$quantité',CURRENT_DATE())");
			        	...
			        	if ($autre =='oui') {
							...
							$q1=mysql_query("SELECT `codeproduit`,`quantité` FROM `vente_produit` WHERE`n_vente`='$nuve'");
							$ppa_total=0;
	    	    		...
	    	    		echo '<script>alert("Pas créer avec succés");</script>';
	    	    	}	
    	    	}	
    	    }
		}
...
<body class="body">
	...
	<div class="content">
		<h1 class="page"> Créer une vente <h1>
        <div class="a">
        	<div class="b">
	        	<table class="table">
	        		<form action="" method="POST">
		        		<tr>
		        			<td colspan="2"><input type="text" name="nuve" placeholder="numéro de vente" autofocus="" onkeypress="Numero()"></td>
		        		</tr>	        			        		
		        		<tr>
